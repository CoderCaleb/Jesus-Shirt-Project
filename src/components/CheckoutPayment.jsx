  ...
  return (
    <div className="w-full h-full md:w-1/2">
      <div className="w-full h-full flex items-center relative justify-center flex-col px-3 sm:px-10 sm:min-w-[400px] py-5">
        {showItems && (
          <div className={`w-full h-full overflow-y-scroll absolute bg-slate-200 md:hidden animate-fade-up`}>
            <div className="flex items-center mx-5">
              <p className="text-3xl font-semibold sm:text-left text-center flex-1 my-5">Your Orders 🛒</p>
              <AiOutlineCloseCircle size={40} className="cursor-pointer" onClick={() => setShowItems(false)} />
            </div>
        ...
        <div className="flex flex-col gap-3 w-full overflow-y-scroll">
          <div className="font-semibold flex items-center justify-between gap-3 text-lg md:text-xl mb-10 w-full">
            <IoMdArrowBack size={22} className="cursor-pointer" onClick={() => setCheckoutProgress(1)} />
            <p className="flex-1">How do you want your order?</p>
          </div>
          <div className="text-base font-semibold flex absolute gap-2 items-center cursor-pointer md:hidden top-5 right-5" onClick={() => setShowItems((prev) => !prev)}>
            <p>My Order</p>
            <FaChevronDown />
          </div>
