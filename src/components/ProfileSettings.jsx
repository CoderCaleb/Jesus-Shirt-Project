...
export default function ProfileSettings() {
  ...
  const { user, userToken, setShowReauthenticateModal, setShowChangePasswordModal } = useContext(StateSharingContext);
  ...
  console.log(fetchedProfileInfo, profileUpdates.birthday);

  const handleChange = handleFieldChange(setProfileUpdates, setFormErrors);

  const handleApiErrors = handleFieldErrors(setApiError);
  ...
  return (
    <div>
      ...
      {!apiError.fetchProfileDataError ? (
        fetchedProfileInfo.current && !isEmptyObject(fetchedProfileInfo) ? (
          <div>
            <div>
            ...
            <div className="">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  onClick={() => setShowChangePasswordModal({state:true})}
                >
                  Password reset
                </button>
                <button
                  className="border-2 px-3 py-2 font-semibold text-sm rounded-[10px] border-black bg-black text-white hover:bg-white hover:text-black"
                  onClick={() => setShowReauthenticateModal({state:true})}
                >
                  Change email
                </button>
              </div>
        ...
        <p className="text-center mt-10">{apiError.fetchProfileDataError}</p>
      )}
    </div>
  );
}
