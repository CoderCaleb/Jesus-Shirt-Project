...
import { capitalizeFirstLetter, formatExpirationDate, addDaysToDate } from "../utils/helpers";
...
const OrderTracking = () => {
  ...
  if (error) {
    return <div className="w-full h-full"><p>{error}</p></div>;
  }

  if (!orderInfo || Object.keys(orderInfo).length === 0) {
    return <div className="w-full h-full"><p>Loading order...</p></div>;
  }

  if (orderInfo.error) {
    return <div className="w-full h-full"><p>{orderInfo.error}</p></div>;
  }
...
const OrderTimeline = ({ timelineStatus, orderDate, index }) => {
  ...
  const timeStatuses = timelineStatus.map((status, idx) => {
    const currentDate = addDaysToDate(prevDate, idx + 2).toLocaleDateString("en-US", options);
      ...
      <div className="flex flex-col text-slate-600 text-sm">
        {paymentData.type === "card" ? (
          <div>
            ...
            <p>{formatExpirationDate(paymentData.card.exp_month, paymentData.card.exp_year)}</p>
          </div>
        ) : (
          <div>
