    ...
    <div className="relative px-6 flex justify-between items-center w-full bg-white shadow-md z-40 h-[64px]">
      {isOpen && (
        <div className="w-full flex gap-7 flex-col bg-white py-5 absolute left-0 top-[64px] cursor-pointer md:hidden">
          <NavbarLink
            text="Home"
            isActive={checkIconActive("")}
            onClick={() => handleNavigation("")}
          />
          <NavbarLink
            text="Cart"
            isActive={checkIconActive("cart")}
            onClick={() => handleNavigation("cart")}
          />
          <NavbarLink
            text="Shop"
            isActive={checkIconActive("shop")}
      ...
      <Link href="/">
        <Image
          src={"/images/jesus-cross-logo.png"}
          alt="logo"
          width={64}
          height={64}
      ...
      <div className="flex gap-5 items-center font-semibold z-10 cursor-pointer justify-center">
        <Navlink
          ...
          clickedFunc={() => handleNavigation("cart")}
        />
        {sessionExist === true && (
        ...
        {pathname === "/" ? (
          ...
          <Navlink
            content="Shop"
            clickedFunc={() => handleNavigation("shop")}
          />
        )}
  ...
  return (
    <div className="group flex justify-center">
      ...
      <div className="hidden flex-col absolute rounded-md shadow-md shadow-slate-400 top-10 group-hover:flex z-10 bg-background">
        <DropdownChoice
          link="Profile"
          clickedFunc={() => router.push("/profile")}
        />
        <DropdownChoice
          link="Orders"
...
const HamburgerMenu: React.FC<HamburgerMenuProps> = ({
  ...
  <div className="md:hidden flex gap-3">
    ...
    >
      <span
        className={`bg-slate-600 block transition-all duration-300 ease-out h-1 w-full rounded-sm ${
          isOpen ? "rotate-45 translate-y-2" : "-translate-y-0.5"
        }`}
      ></span>
      <span
        className={`bg-slate-600 block transition-all duration-300 ease-out h-1 w-full rounded-sm my-0.5 ${
